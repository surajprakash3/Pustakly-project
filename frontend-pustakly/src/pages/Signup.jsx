...
export default function Signup() {
  ...
  const getPasswordStrength = (value) => {
    ...
    if (!value) return { label: 'Add a password', percent: 0, tone: 'weak' };
    if (score <= 2) return { label: 'Weak', percent: 35, tone: 'weak' };
    if (score === 3) return { label: 'Fair', percent: 60, tone: 'fair' };
    if (score === 4) return { label: 'Good', percent: 80, tone: 'good' };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setErrors((prev) => ({ ...prev, form: error.message || 'Registration failed. Please try again.' }));
    }
  };
